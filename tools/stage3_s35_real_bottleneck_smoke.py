#!/usr/bin/env python3
"""Run one bounded real-network S3.5 Bottleneck model integration smoke.

Successful execution performs exactly two model calls:

1. classify typed agent-safe PPA evidence and propose evidence-linked hypotheses;
...
``optimize/full`` execution.  Acceptance proves only the S3.5 evidence,
classification, prompt, response, and complete-source integration contract.
"""
...
EXPECTED_BASELINE = "c881dea0ea68f0fbf7c2b359bd270f362827a90f"
DEFAULT_MODEL = "deepseek-v4-flash"
DEFAULT_SOURCE = "tests/fixtures/stage3_s35/bottleneck_smoke_kernel.cpp"
DEFAULT_TOP = "s35_bottleneck_top"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the bounded S3.5 real Bottleneck model smoke."
    )
...
def main() -> int:
    ...
    artifacts = BottleneckModelArtifactWriter(output_root / "optimizer")
    task = TaskSpec(
    ...
    request = HypothesisRequest(
        ...
        safe_context={
            "policy": "safe-v1",
            "objective": "latency",
            "smoke_scope": "bottleneck_model_contract_only",
    ...
    if not hypotheses:
        raise RuntimeError(
            "real model returned no executable Bottleneck hypothesis for the explicit II fixture"
        )
    ...
    classifications = [item.to_dict() for item in provider.classifications]
    model_lines = artifacts.path.read_text(encoding="utf-8").splitlines()
    summary = {
        "schema_version": 1,
        "accepted": True,
        "claim_scope": "bottleneck_model_contract_only",
    ...
    print("S35_REAL_BOTTLENECK_SMOKE_ACCEPTED=true")
    ...
    print("CLASSIFICATION_AUTHORITATIVE=false")
    print("RAW_REPORT_USED=false")
    print("STATIC_BOTTLENECK_GATE_USED=false")
...
def smoke_model_parameters(*, family: str, max_tokens: int) -> dict[str, Any]:
    ...
    The strict Bottleneck JSON schema remains enforced by
    ``BottleneckAnalysisResponseContract``.  DeepSeek's provider-side JSON
...
def smoke_ppa() -> PpaEvidence:
    return PpaEvidence(
        evidence_id="ppa-s35-smoke",
        parser_profile="s35-smoke-fixture-v1",
        report_format=PpaReportFormat.XML,
        report_relative_path="fixtures/s35_smoke_csynth.xml",
        report_sha256=sha256(b"s35 typed smoke PPA fixture").hexdigest(),
...
def require_repo(repo: Path) -> None:
    ...
    required = (
        repo / "agrefactor" / "optimization" / "bottleneck_model.py",
        repo / "agrefactor" / "prompts" / "optimization.py",
    )
    for path in required:
        if not path.is_file():
            raise FileNotFoundError(f"S3.5 source is missing: {path}")
...
def verify_summary(summary: dict[str, Any]) -> None:
    ...
    if usage["llm_calls"] != 2:
        raise AssertionError("real S3.5 smoke must use exactly two LLM calls")
    for key in ("tool_calls", "compile_calls", "csim_calls", "csynth_calls"):
        if usage[key] != 0:
            raise AssertionError(f"real S3.5 smoke unexpectedly consumed {key}")
    if summary["classification_authoritative"]:
        raise AssertionError("model classification must remain non-authoritative")
    if summary["raw_report_used"]:
        raise AssertionError("S3.5 smoke must use only typed evidence projection")
    if summary["static_bottleneck_gate_used"]:
        raise AssertionError("S3.5 must not use a static bottleneck gate")
    ...
    if summary["output_token_policy"] != "stage2_typed_output_policy":
        raise AssertionError("S3.5 must reuse the Stage 2 typed output policy")
    ...
    if summary["product_optimize_full_enabled"]:
        raise AssertionError("S3.5 must not enable product optimize/full")


def default_output_root() -> Path:
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return Path(
        f"/data/agrefactor_runs/stage3_s35_real_bottleneck_smoke_{stamp}_{os.getpid()}"
    )
...
if __name__ == "__main__":
    try:
        ...
        print("S35_REAL_BOTTLENECK_SMOKE_ACCEPTED=false", file=sys.stderr)
