...
TOOL = REPO / "tools" / "stage3_s36_real_pragma_smoke.py"


class PragmaSmokeToolTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.module = runpy.run_path(str(TOOL), run_name="stage3_s36_smoke_test")

    def test_direct_tool_bootstrap_and_inside_repository_source(self):
        source = self.module["resolve_source"](
            REPO,
            "tests/fixtures/stage3_s36/pragma_smoke_kernel.cpp",
        )
        self.assertTrue(source.is_file())
        self.assertEqual(source.name, "pragma_smoke_kernel.cpp")
    ...
    def test_smoke_ppa_has_explicit_ii_and_fixture_identity(self):
        ...
        self.assertEqual(evidence.evidence_id, "ppa-s36-smoke")
    ...
    def test_deepseek_parameters_disable_thinking_without_provider_json_mode(self):
        ...
        ceiling = self.module["smoke_model_parameters"](
            family="deepseek",
            max_tokens=65536,
        )
        self.assertEqual(ceiling["max_tokens"], 65536)
        with self.assertRaises(ValueError):
    ...
    def test_non_deepseek_parameters_preserve_provider_default_thinking(self):
        ...
        self.assertEqual(parameters, {"temperature": 0, "max_tokens": 4096})

    def test_verify_summary_requires_two_llm_and_no_tools(self):
        summary = {
            "budget_usage": {
                "llm_calls": 2,
                "tool_calls": 0,
        ...
        bad = dict(summary)
        bad["action_authoritative"] = True
        with self.assertRaises(AssertionError):
            self.module["verify_summary"](bad)

    def test_expected_baseline_is_s35_closure(self):
        self.assertEqual(
            self.module["EXPECTED_BASELINE"],
            "f5a46d62cca864828e6d1ec3bbe7c5b2ef200f8a",
        )
