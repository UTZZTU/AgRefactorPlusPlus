"""Fail-closed provenance checks for the future R4 integration seam."""
...
def _mapping(value: Mapping[str, Any], name: str) -> dict[str, Any]:
    ...
    return json.loads(json.dumps(dict(value), ensure_ascii=False, allow_nan=False, sort_keys=True))
...
def validate_r4_provenance(*, event: Mapping[str, Any], execution_identity: Mapping[str, Any], advisory: Mapping[str, Any], gate: Mapping[str, Any], revision: Mapping[str, Any], authorization: Mapping[str, Any], canary: Mapping[str, Any], candidate: str, original: str, testbench_hashes: Mapping[str, str]) -> R4ProvenanceResult:
    reasons: list[str] = []
    ...
    if execution_identity.get("identity_complete") is not True or execution_identity.get("hidden_input_count", 0) != 0 or execution_identity.get("secret_present", False) or execution_identity.get("private_reasoning_present", False):
    ...
    for field in ("policy_decision_id", "budget_reservation_id", "deterministic_terminal_ref"):
        if not isinstance(authorization.get(field), str) or not authorization[field].strip():
            reasons.append(f"authorization_producer_ref_missing:{field}")
    if canary.get("enabled") is not True or canary.get("operator_enabled") is not True:
        reasons.append("canary_not_operator_enabled")
    for key in ("target_identity", "toolchain_identity", "parser_identity", "model_identity", "prompt_sha256"):
    ...
    if event.get("evidence_view", "agent_safe") != "agent_safe" or event.get("hidden_input_count", 0) != 0 or event.get("secret_present", False) or event.get("private_reasoning_present", False) or event.get("physical_tool_launched") is not True or event.get("evidence_complete") is not True:
        reasons.append("agent_safe_boundary_violated")
    if not isinstance(testbench_hashes, Mapping) or not testbench_hashes:
        reasons.append("testbench_identity_missing")
    elif any(not isinstance(value, str) or len(value) != 64 or any(char not in "0123456789abcdef" for char in value.casefold()) for value in testbench_hashes.values()):
        reasons.append("testbench_identity_invalid")
    refs = tuple(sorted(event_refs))
    return R4ProvenanceResult(valid=not reasons, reasons=tuple(dict.fromkeys(reasons)), checked_refs=refs)


__all__ = ["R4ProvenanceResult", "validate_r4_provenance"]
