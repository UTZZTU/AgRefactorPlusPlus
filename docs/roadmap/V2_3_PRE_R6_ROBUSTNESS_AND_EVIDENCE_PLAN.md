...
9. **预算约束**：继续受累计 500 Provider / 500 Vitis 硬上限约束，除非项目所有者
   另行明确授权。任何计划都必须先做 worst-case reserve 再运行。
...
动作：冻结当前 HEAD、R5 证据归档、500/500 ledger、模型/工具身份和本文版本；确认
R5/R6 状态不变。
...
当前累计消耗为 Provider `248/500`、Vitis `111/500`，剩余 `252/389`。本文只给出
第一轮上限，实际执行前必须由 machine-readable preflight 重新计算：

| 工作 | Provider 上限 | Vitis 上限 |
|---|---:|---:|
| P0-P3 inventory、去重、许可、静态/host smoke | 0 | 10 |
| P4 source-baseline 资格筛选 | 0 | 120 |
| P5 广覆盖 ordinary refactor | 126 | 80 |
| P6-P8 机会诊断、通用修正和 observation 因果验证 | 76 | 100 |
| P9/P10 独立复现、审计和恢复保留 | 25 | 40 |
| **第一轮新增上限** | **227** | **350** |

该分配保留 Provider 25 次、Vitis 39 次未分配缓冲，并且不超过累计 500/500。真实
调用以 ledger 为准，未用额度不能自动跨阶段挪用。如果合格案例数量超过剩余预算，
先完成零 Provider 的资格与去重，提交覆盖/功效分析和新增预算申请；不得静默超限，
也不得为了省预算挑选已知容易成功的案例。
