...
void loadMatrixToStream(int& inputStream, int& matrixStream, int rows, int cols) {
    for (int i = 0; i < rows * cols; ++i) {
        matrixStream = inputStream;
    }
}

void loadMatrixFromStream(int& matrixStream, int matrix[MAX_SIZE][MAX_SIZE], int rows, int cols) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            matrix[i][j] = matrixStream;
        }
    }
}
...
void process_top(Command& cmdStream, int& inputStream, int& outputStream) {
    Command cmd;
    int matrixStream; 
